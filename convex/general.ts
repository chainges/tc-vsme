...
const createGeneralForm = (Data:any[])=>{
...
export const getGeneralFormByOrgIdInternal = internalQuery({
  ...
  handler: async (ctx, { orgId, reportingYear }) => {
    const org = await ctx.db
      ...
      .collect();
    return createGeneralForm(org);
    
  },
});

// Public action — this is what your Python backend will call
export const getGeneralFormDataForXbrl = action({
  args: { orgId: v.string(), reportingYear: v.number() },
  returns: v.any(),
